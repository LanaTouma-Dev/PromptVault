...
@Component({
  ...
  styles: [`
    ...
    label { display: block; font-size: 12px; font-weight: 600; color: var(--text-muted); margin-bottom: 6px; letter-spacing: .2px; text-transform: uppercase; }
    ...
    .pricing-opt {
      ...
      transition: background 140ms, color 140ms; color: var(--text-muted);
    }
    ...
    .step-pip {
      ...
      background: var(--border-mid); transition: background 250ms, width 250ms;
    }
      ...
      <div class="sheet" (click)="$event.stopPropagation()">
          ...
          <div class="panel" [class.active]="step() === 1">
            ...
            <div style="margin-top:18px">
              ...
              <div class="add-custom-row">
                <input type="text" [(ngModel)]="customModelName" placeholder="e.g. Qwen 2.5, Yi-34B…" />
          ...
          <div class="panel" [class.active]="step() === 2">
            ...
            @if (detectedVars().length) {
              <div style="margin-top:10px">
                <p style="font-size:11px; font-weight:600; color:var(--text-muted); text-transform:uppercase; letter-spacing:.3px; margin-bottom:6px;">
                  Detected variables
                </p>
                <div class="vars-row">
        ...
        <div class="modal-footer">
          ...
          @if (step() < 2) {
            ...
            <button class="btn-nav btn-next" (click)="submit()" [disabled]="saving">{{ saving ? 'Publishing…' : 'Publish →' }}</button>
          }
        </div>
      </div>
    </div>
...
export class AddPromptModalComponent {
  ...
  badgeClass(p: AITool['pricing']): string { return 'model-option-badge badge-' + p; }
